...
class AutoElectsysConfig:
    ...
    def init_root(self):
        ...
        self.root.title('AutoElectsys配置设置')
        self.root.geometry('552x400')
    ...
    def init_widgets(self):
        self.tab = ttk.Notebook(self.root, width = self.tab_w, height = self.tab_h)
        self.tab.place(x = self.tab_x, y = self.tab_y)

        self.login_frame = Tkinter.Frame(self.tab, background = self.login_color)
        self.tab.add(self.login_frame, text = '登录设置')

        self.pswd_choice_v = Tkinter.IntVar()
        self.pswd_chorme_radio = ttk.Radiobutton(self.login_frame, text = '使用Chrome保存的用户名和密码', variable = self.pswd_choice_v, value = 1, command = self.on_pswd, style = 'Login.TRadiobutton')
        self.pswd_chorme_radio.place(x = self.x1, y = self.y1)

        self.pswd_custom_radio = ttk.Radiobutton(self.login_frame, text = '使用自定义的用户名和密码', variable = self.pswd_choice_v, value = 0, command = self.on_pswd, style = 'Login.TRadiobutton')
        ...
        self.captcha_v = Tkinter.IntVar()
        self.captcha_check = ttk.Checkbutton(self.login_frame, text = '自动识别验证码', variable = self.captcha_v, command = self.on_captcha, style = 'Login.TCheckbutton')
        self.captcha_check.place(x = self.x1, y = self.y5)

        self.relogin_v = Tkinter.IntVar()
        self.relogin_check = ttk.Checkbutton(self.login_frame, text = '自动重新登录', variable = self.relogin_v, command = self.on_relogin, style = 'Login.TCheckbutton')
        ...
        self.relogin_interval_v = Tkinter.StringVar()
        ...
        self.course_frame = Tkinter.Frame(self.tab, background = self.course_color)
        ...
        self.course_id_v = Tkinter.StringVar()
        ...
        self.teacher_row_v = Tkinter.StringVar()
        ...
        self.round_v = Tkinter.StringVar()
        self.round_combo = ttk.Combobox(self.course_frame, values = course_rounds, textvariable = self.round_v, state = 'readonly', width = self.w1)
        self.round_combo.bind('<<ComboboxSelected>>', self.on_round)
        self.round_combo.place(x = self.course_x2, y = self.y3)

        self.autolocate_v = Tkinter.IntVar()
        self.autolocate_check = ttk.Checkbutton(self.course_frame, text = '自动定位选课页面', variable = self.autolocate_v, command = self.on_locate, style = 'Course.TCheckbutton')
        ...
        self.first_cat_v = Tkinter.StringVar()
        self.first_cat_combo = ttk.Combobox(self.course_frame, values = first_categories, textvariable = self.first_cat_v, state = 'readonly', width = self.w1)
        ...
        self.second_cat_v = Tkinter.StringVar()
        ...
        self.misc_frame = Tkinter.Frame(self.tab, background = self.misc_color)
        ...
        self.sleep_v = Tkinter.StringVar()
        ...
        self.ok_button = ttk.Button(self.root, text = '确定', command = self.store)
        self.ok_button.place(x = self.bottom_x1, y = self.bottom_y)
        self.root.bind('<Control-s>', self.store)

        self.cancel_button = ttk.Button(self.root, text = '取消', command = self.ask_quit)
        self.cancel_button.place(x = self.bottom_x2, y = self.bottom_y)
        self.root.bind('<Escape>', self.ask_quit)

    def init_tips(self):
        self.pswd_tip_label = ttk.Label(self.login_frame, style = 'Info_Login.TLabel')

        self.captcha_tip_label = ttk.Label(self.login_frame, text = '自动识别 jAccount 登录页面的验证码。', style = 'Info_Login.TLabel')
        self.captcha_tip_label.place(x = self.tip_x, y = self.y5)

        self.relogin_tip_label = ttk.Label(self.login_frame, text = '每隔一段时间自动重新登录，\n刷新 Cookie 以防止页面过期。\n需开启自动定位选课页面功能。', style = 'Info_Login.TLabel')
        self.relogin_tip_label.place(x = self.tip_x, y = self.y6)

        self.course_id_tip_label = ttk.Label(self.course_frame, text = '欲选课程的课程代码，可在网页上查知。', style = 'Info_Course.TLabel')
        self.course_id_tip_label.place(x = self.tip_x, y = self.y1)

        self.teacher_row_tip_label = ttk.Label(self.course_frame, text = '欲选教师或时间段在内层页面中第几行。', style = 'Info_Course.TLabel')
        self.teacher_row_tip_label.place(x = self.tip_x, y = self.y2)

        self.round_tip_label = ttk.Label(self.course_frame, text = '“其他”项用于二专选课等。\n此模式下不支持自动定位选课页面。', style = 'Info_Course.TLabel')
        self.round_tip_label.place(x = self.tip_x, y = self.y3)

        self.autolocate_tip_label = ttk.Label(self.course_frame, text = '按照课程的类型，自动进入所在的页面。', style = 'Info_Course.TLabel')
    ...
    def on_round(self, event = None):
        if self.round_v.get() == course_rounds[0].decode('UTF-8'):
    ...
    def on_first_cat(self, event = None):
        ...
        if s in [first_categories[0].decode('UTF-8'), first_categories[4].decode('UTF-8')]:
            new_state = 'disabled'
            new_text = ''
        else:
            new_state = 'normal'
            if s == first_categories[1].decode('UTF-8'):
                new_text = '填写课程所属的模块名称，\n如“个性化教育”等。\n请填写网页上显示的全称，不要简写。'
            elif s == first_categories[2].decode('UTF-8'):
                new_text = '填写课程所属的模块名称，\n如“人文学科”“社会科学”等。\n请填写网页上显示的全称，不要简写。'
            else:
                new_text = '填写开课院系名称。\n请填写网页上显示的全称，不要简写。\n注：默认选择本年级，跨年级选课\n请勿使用自动定位功能。'
    ...
    def load(self):
        ...
        try:
            self.pswd_choice_v.set(bool(cf.getint('Login', 'password_saved')))
        except:
            self.config_file_valid = False
            self.pswd_choice_v.set(default_pswd_choice)

        try:
            self.captcha_v.set(bool(cf.getint('Login', 'auto_captcha')))
        except:
            self.config_file_valid = False
            self.captcha_v.set(default_captcha)

        try:
            cf_relogin_interval = cf.getint('Login', 'relogin_interval')
            if cf_relogin_interval == 0:
                self.relogin_interval_v.set('')
                self.relogin_v.set(0)
            elif cf_relogin_interval > 0:
        ...
        try:
            self.course_id_v.set(cf.get('CourseInfo', 'course_id'))
        except:
            self.config_file_valid = False
            self.course_id_v.set('')

        try:
            cf_teacher_row = cf.getint('CourseInfo', 'teacher_row')
            if cf_teacher_row > 0:
        ...
        try:
            cf_round = cf.getint('CourseLocate', 'round')
            if cf_round not in range(0, 7):
                self.config_file_valid = False
                cf_round = default_round
        ...
        try:
            self.autolocate_v.set(bool(cf.getint('CourseLocate', 'auto_locate')))
        except:
            self.config_file_valid = False
            self.autolocate_v.set(0)
            self.autolocate_tip_label.config(state = 'disabled')
        ...
        try:
            cf_first_category = cf.getint('CourseLocate', 'first_category')
            if cf_first_category not in [1, 2, 3, 4, 5]:
                self.config_file_valid = False
                cf_first_category = default_first_category
            self.first_cat_v.set(first_categories[cf_first_category - 1])
            if cf_first_category in [2, 3, 4]:
                try:
                    self.second_cat_v.set(cf.get('CourseLocate', 'second_category'))
                except:
                    self.config_file_valid = False
        ...
        try:
            cf_sleep = cf.getint('Miscellaneous', 'sleep_time')
            if cf_sleep > 0:
        ...
        try:
            ...
            try:
                username = faccount.readline().replace('\n', '')
                password = base64.decodestring(faccount.readline().replace('\n', ''))
            except:
                username = ''
                password = ''
        except:
    ...
    def show_status(self):
        if self.config_file_exists:
            if self.config_file_valid:
                self.status_label = ttk.Label(self.root, text = '成功读取配置文件', style = 'Success.TLabel')
            else:
                self.status_label = ttk.Label(self.root, text = '配置文件格式错误或值无效，将自动修正', style = 'Error.TLabel')
        else:
            self.status_label = ttk.Label(self.root, text = '配置文件不存在，将创建新的配置文件', style = 'Error.TLabel')
        self.status_label.place(x = self.status_x, y = self.status_y, anchor = Tkinter.N)
    ...
    def store(self, event = None):
        ...
        if not cf.has_section('Login'):
            cf.add_section('Login')
        if not cf.has_section('CourseInfo'):
            cf.add_section('CourseInfo')
        if not cf.has_section('CourseLocate'):
            cf.add_section('CourseLocate')
        if not cf.has_section('Miscellaneous'):
            cf.add_section('Miscellaneous')

        cf.set('Login', 'password_saved', str(self.pswd_choice_v.get()))

        cf.set('Login', 'auto_captcha', str(self.captcha_v.get()))

        if self.relogin_v.get():
            cf_relogin_interval = self.relogin_interval_v.get().strip()
            if cf_relogin_interval == '':
                tkMessageBox.showwarning('错误', '请设置自动重新登录的间隔时间！')
                return
            if not is_positive_int(cf_relogin_interval):
                tkMessageBox.showwarning('错误', '自动重新登录间隔时间应为正整数！')
                return
        else:
            cf_relogin_interval = '0'
        cf.set('Login', 'relogin_interval', cf_relogin_interval)

        cf.set('CourseInfo', 'course_id', self.course_id_v.get().strip().encode('UTF-8'))

        cf_teacher_row = self.teacher_row_v.get().strip()
        if cf_teacher_row == '':
            tkMessageBox.showwarning('错误', '请填写教师行数！')
            return
        if not is_positive_int(cf_teacher_row):
            tkMessageBox.showwarning('错误', '教师行数应为正整数！')
            return
        cf.set('CourseInfo', 'teacher_row', cf_teacher_row)

        cf.set('CourseLocate', 'round', str(course_rounds.index(self.round_v.get().encode('UTF-8'))))

        cf.set('CourseLocate', 'auto_locate', str(self.autolocate_v.get()))

        cf.set('CourseLocate', 'first_category', str(first_categories.index(self.first_cat_v.get().encode('UTF-8')) + 1))

        cf.set('CourseLocate', 'second_category', self.second_cat_v.get().strip().encode('UTF-8'))

        cf_sleep = self.sleep_v.get().strip()
        if cf_sleep == '':
            tkMessageBox.showwarning('错误', '请设置选课页面刷新间隔！')
            return
        if not is_positive_int(cf_sleep):
            tkMessageBox.showwarning('错误', '选课页面刷新间隔应为正整数！')
            return
        cf.set('Miscellaneous', 'sleep_time', cf_sleep)

        if not tkMessageBox.askokcancel('保存', '确定要保存当前配置吗？'):
            return

        try:
            fcf = open(config_file_name, 'w+')
            cf.write(fcf)
        except:
            tkMessageBox.showerror('错误', '无法写入配置文件！')
            self.quit()

        username = self.user_v.get().encode('UTF-8')
        password = self.pass_v.get().encode('UTF-8')
        if username == '' and password == '':
            ...
            try:
                faccount = open(pswd_file_name, 'w')
                faccount.write(username + '\n')
                faccount.write(base64.encodestring(password) + '\n')
            except:
                tkMessageBox.showerror('错误', '无法写入账号信息文件！')
                self.quit()
    ...
    def ask_quit(self, event = None):
        if tkMessageBox.askokcancel('退出', '确定要退出吗？'):
            self.quit()
